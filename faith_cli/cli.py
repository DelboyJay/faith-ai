...
def _ensure_framework_home() -> None:
    ...
    _copy_tree(source_config / "archetypes", archetypes_dir())

    # Keep a snapshot for future packaged installs, but the current PoC runs from the repo compose file.
    if not installed_compose_file().exists():
        shutil.copy2(source_compose_file(), installed_compose_file())

    if (source_config / ".env.template").exists() and not env_file().exists():
        shutil.copy2(source_config / ".env.template", env_file())
    if (source_config / "secrets.yaml.template").exists() and not secrets_file().exists():
        shutil.copy2(source_config / "secrets.yaml.template", secrets_file())
